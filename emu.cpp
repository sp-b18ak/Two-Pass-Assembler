...
vector<string> code;                    // StoreS the code
...
void ldc(int value){                    //Functions to perform as per Mnemonic                     
    B = A;                              //Load accumulator with the value specified 
    A = value;
}
...
string decToHex(unsigned int num){          // convert dec to hex
    stringstream ss; 
    ss << hex << num; 
    string conv = ss.str();
...
void inputMacCode(){                        // Input function
    string s;
...
void entryMsg(){
    cout << "Implement the following:-" << endl;
...
void showReg(){                     // Show the register
...
bool runCode(int oper, int times = (1 << 25)){                  // run the code or perform operations
    int iter = 0;
...
bool runEmu(){                                      // Interactive emulator run
